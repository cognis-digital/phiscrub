"""Native cognis-connect emit for phiscrub — forward findings to any platform.

Maps phiscrub's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    phiscrub ... --format json | phiscrub-emit --to stix
    phiscrub-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['description'] = rec.get('match', '')
        out['type'] = 'phishing' if 'phishing' in rec.get('detector', '') else 'unknown'
        out['severity'] = 'medium'
        out['tags'] = ['phishing']

...
**phiscrub** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `phiscrub` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
phiscrub ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
phiscrub ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
phiscrub ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `phiscrub` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="phiscrub") for rec in records]   # records = your JSON output
